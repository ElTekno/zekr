...
RxDouble mushafFontSize = 40.0.obs;
...
Future saveSettings() async {
  ...
  await prefs.setInt('arabicFontsSize',arabicFontSize.toInt());
  await prefs.setInt('mushafFontsSize',mushafFontSize.toInt());
}

Future getSettings() async {
  try{
    final prefs = await SharedPreferences.getInstance();
    arabicFontSize.value = await prefs.getInt('arabicFontsSize')!.toDouble();
    mushafFontSize.value = await prefs.getInt('mushafFontsSize')!.toDouble();
  }
  catch (_) {
    double arabicFontsSize=28;
    double mushafFontsSize=40;
  }
}
...
Future readJson () async {
  final String response = await rootBundle.loadString("assets/json/quran/hafs_smart_v8.json");
  final data = json.decode(response);
  return quran = data['quran'];
}
